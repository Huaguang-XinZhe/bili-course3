...
const CourseCard: React.FC<CourseCardProps> = ({ course, listView }) => {
  ...
  return (
    <div
      className={`${
        listView ? "flex gap-4 w-full h-[116px]" : "flex flex-col w-[236px]"
      } bg-white overflow-hidden cursor-pointer`}
      onClick={handleClick}
    >
      <div
        className={`${
          listView ? "w-[206px] h-[116px]" : "w-[236px] h-[133px]"
      ...
      <div className={`${listView ? "flex-1" : ""} p-3 flex flex-col`}>
        ...
        {listView && course.first_ep_label && course.first_ep_title && (
          <div className="flex items-center gap-1 text-xs text-[#00A1D6] mt-1">
            <span className="border border-[#00A1D6] px-1 rounded text-[10px]">
